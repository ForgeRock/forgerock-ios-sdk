...
public class KbaCreateCallback: MultipleValuesCallback {
    ...
    required init(json: [String : Any]) throws {
        ...
        for output in outputs {
            if let name = output["name"] as? String, name == "predefinedQuestions", let questions = output["value"] as? [String] {
                self.predefinedQuestions = questions
            }
        }
        
        for input in inputs {
            if let name = input["name"] as? String, name.contains("question") {
                self.questionInputKey = name
            } else if let name = input["name"] as? String, name.contains("answer") {
